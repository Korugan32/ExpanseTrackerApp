...
class SearchStockSymbolsUseCase @Inject constructor(private val stockRepository: StockRepository) {
    fun getStockSearchSymbols(q: String): Flow<Resource<StockSymbolSearchDto>> = flow {
        try {
            emit(Resource.Loading())
            val stock = stockRepository.getSearchStockSymbols(q)
            if (stock.result.isNotEmpty()) {
